...
#' @return z tibble
#' 
perf_table_common_ETF<-function(country="america", exchange="AMEX", valueTraded=10000000){
  ...
  get_performance_table <- function() {
    adat <- get_tradingview_data_from_json_string(paste0('{"filter":[{"left":"type","operation":"in_range","right":["fund"]},{"left":"subtype","operation":"in_range","right":["etf"]},{"left":"exchange","operation":"in_range","right":["',exchange,'"]}],"options":{"lang":"en"},"symbols":{"query":{"types":[]},"tickers":[]},"columns":["logoid","name","change|1","change|5","change|15","change|60","change|240","change","change_from_open","Perf.W","Perf.1M","Perf.3M","Perf.6M","Perf.YTD","Perf.Y","beta_1_year","SMA10","SMA20" ,"SMA50", "SMA200","Value.Traded", "VWAP","ATR" ,"average_volume_30d_calc", "ebitda","ebitda_yoy_growth_fy","ebitda_qoq_growth_fq","ebitda_yoy_growth_fq","net_income_yoy_growth_fy","net_income_qoq_growth_fq","net_income_yoy_growth_fq","net_income_yoy_growth_ttm","earnings_per_share_diluted_yoy_growth_ttm", "return_on_equity","after_tax_margin" ,"Mom","Recommend.All", "Volatility.D","RSI","number_of_employees","market_cap_basic","sector","industry","country","Low.6M","High.All","Low.All","High.1M","High.3M","High.6M", "price_52_week_high","price_earnings_ttm","close","description","earnings_release_next_date","premarket_change_abs","premarket_change","premarket_volume","postmarket_change","name","type","subtype","update_mode","RSI","RSI[1]","pricescale","minmov","fractional","minmove2"],"sort":{"sortBy":"market_cap_basic","sortOrder":"desc"},"range":[0,6550]}'))
    return(adat)}
